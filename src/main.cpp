...
// Runtime Config
#include "runtimeConfig/runtimeConfigService.h"
...
#pragma region RuntimeConfig
RuntimeConfigService& runtimeConfigService = RuntimeConfigService::getInstance();
...
void initManager() {
    ...
    manager.addMessageService(&runtimeConfigService);
    ESP_LOGV(TAG, "Runtime Configuration service added to manager");
...
#ifdef RUNTIME_CONFIG_ENABLED
    // Initialize RuntimeConfig
    initRuntimeConfig();
    ESP_LOGV(TAG, "Heap after init RuntimeConfig: %d", ESP.getFreeHeap());
#endif
