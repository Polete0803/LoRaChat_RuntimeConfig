...
void LoRaMeshService::loopReceivedPackets() {
    ...
    while (radio.getReceivedQueueSize() > 0) {
        ...
        ESP_LOGV(LMS_TAG, "After createDataMessage");

        // Process the packet
        MessageManager::getInstance().processReceivedMessage(LoRaMeshPort, message);

        ESP_LOGV(LMS_TAG, "After processReceivedMessage");
...
void LoRaMeshService::createReceiveMessages() {
    int res = xTaskCreate(processReceivedPackets, "Receive App Task", 6000, (void*)1, 2,
